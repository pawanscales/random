...
const CallLayout: React.FC<CallLayoutProps> = ({ callName }) => {
  ...
  const [isSpeakerOn, setIsSpeakerOn] = useState(true);  // Default speaker is ON
  const [isMicOn, setIsMicOn] = useState(true);  // Default mic is ON
  ...
  const router = useRouter(); // Ensure router is used inside the page component
        ...
        <div className={styles.callContainer}>
          ...
          <div className={styles.controls}>
            <button className={styles.controlBtn} onClick={toggleMic}>
              {isMicOn ? <FaMicrophone color="white" size={24} /> : <FaMicrophone color="gray" size={24} />}
            </button>
